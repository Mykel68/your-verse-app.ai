...
export const VerseSelector = ({
  ...
  const getRandomVerse = async () => {
    ...
    try {
      ...
      const randomChapter =
        Math.floor(Math.random() * randomBook.maxChapter) + 1;
  ...
  return (
    <Card className="p-6 gradient-card border shadow-card hover:shadow-divine transition-all duration-500">
      <Tabs defaultValue="random" className="w-full">
        ...
        <TabsContent value="popular" className="space-y-6 animate-fade-in px-2">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm">
              Timeless verses that inspire millions
            </p>
          </div>
          <div className="space-y-3 max-h-72 overflow-y-auto pr-2 scrollbar-thin scrollbar-track-muted/20 scrollbar-thumb-muted/40">
            {popularVerses.map((verse, index) => (
              <div
                key={index}
                className="w-full text-left p-4 h-auto transition-all duration-300 hover:shadow-md group border rounded-lg cursor-pointer"
                onClick={() => onVerseSelected(verse)}
                style={{ animationDelay: `${index * 0.05}s` }}
              >
                <div className="space-y-2">
                  <p className="text-sm leading-relaxed verse-text ">
        ...
        <TabsContent value="custom" className="space-y-6 animate-fade-in">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm">
              Share your favorite verse or inspirational quote
            </p>
          </div>
